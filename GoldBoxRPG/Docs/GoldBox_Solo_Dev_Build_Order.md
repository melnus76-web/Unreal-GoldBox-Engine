...
| 42 | **GB-039** (in progress) | Full condition system — add Poisoned, Blinded, Quickened, Slowed, Diseased, Sapped, Petrified. TickConditions each round. Enum already renamed under GB-079 — this ticket is the actual tick/effect logic. **Done:** `TickConditions(CombatantID)` built on `BP_CombatManager` with ForEach -> Switch on E_Condition dispatch; Poisoned case: `ResolveSave(Fortitude)` -> 1d4 damage on fail, wired into `OnActionComplete` before `StartPlayerTurn`/`StartEnemyTurn`; tested and working in PIE. **Remaining:** Blinded (-4 SN/-4 DR/-4 Reflex), Quickened (+1 attack/+1 move), Slowed (-1 attack/-1 move), Unconscious/Paralysed (skip turn guard), Diseased/Sapped/Petrified (post-VS). |
